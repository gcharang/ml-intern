...
GPU sandbox smoke test:   <hardware + result, or explicitly N/A because ...>
...
Packages to install:      <bitsandbytes, etc. — anything not preinstalled; prefer Hub kernels over pip-installing flash-attn>
```
...
**GPU sandbox preflight (mandatory):** if the job runs on GPU, or the script loads a model or uses CUDA, bf16/fp16, quantization, flash attention, or `torch.compile`, you MUST run a `sandbox_create` GPU smoke test (same imports, model-loading path, entrypoint, tiny subset) and fix failures before calling `hf_jobs`. If you skip it, say why in the `GPU sandbox smoke test` checklist line above.

**Hooks will gate this call:** GPU jobs always prompt for confirmation; scheduled jobs always prompt (even under `ML_INTERN_YOLO=1`). CPU jobs prompt by default (override with `ML_INTERN_CONFIRM_CPU_JOBS=0`). That is expected — present the pre-flight check clearly so the user can approve in one read.
